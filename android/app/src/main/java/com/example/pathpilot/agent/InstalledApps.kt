...
/**
 * 런처에 뜨는(=사용자가 열 수 있는) 앱 목록. 서버가 이 목록에서 대상 앱을 고른다.
 *
 * **AndroidManifest에 `<queries>` 선언이 없으면 Android 11+에서 빈 목록이 돌아온다.**
 * 그러면 서버가 고를 후보가 없어 첫 스텝부터 실패한다 — 가장 빠뜨리기 쉬운 함정이다.
 */
    ...
    fun list(context: Context): List<InstalledApp> {
        ...
        return pm.queryIntentActivities(launcherIntent, 0)
            ...
            .mapNotNull { resolveInfo ->
                ...
                InstalledApp(packageName = packageName, label = resolveInfo.loadLabel(pm).toString())
            }
